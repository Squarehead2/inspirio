// src/pages/QuotesFeelingDown.js
import React, { useState, useEffect } from "react";
import Header from "../components/Header";
import { feelingDownQuotes } from "../data/downQuotesData"; // Import feeling down quotes

function QuotesFeelingDown() {
  const [currentQuote, setCurrentQuote] = useState(feelingDownQuotes[0]);
  const [animationClass, setAnimationClass] = useState("");

  const getRandomQuote = () => {
    if (feelingDownQuotes.length > 0) {
      setAnimationClass("slide-out");

      setTimeout(() => {
        const randomIndex = Math.floor(
          Math.random() * feelingDownQuotes.length
        );
        setCurrentQuote(feelingDownQuotes[randomIndex]);
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-lg md:text-xl mb-10 font-light max-w-2xl mx-auto text-gray-200">
              Here are some quotes to lift your spirits.
            </p>
          </div>
...
export default QuotesFeelingDown;
