// src/pages/QuotesInsightful.js
import React, { useState, useEffect } from "react";
import Header from "../components/Header";
import { insightfulQuotes } from "../data/quotesInisghtData"; // Import insightful quotes

function QuotesInsightful() {
  const [currentQuote, setCurrentQuote] = useState(insightfulQuotes[0]);
  const [animationClass, setAnimationClass] = useState("");

  const getRandomQuote = () => {
    if (insightfulQuotes.length > 0) {
      setAnimationClass("slide-out");

      setTimeout(() => {
        const randomIndex = Math.floor(Math.random() * insightfulQuotes.length);
        setCurrentQuote(insightfulQuotes[randomIndex]);
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-lg md:text-xl mb-10 font-light max-w-2xl mx-auto text-gray-200">
              Here are some quotes to inspire your thoughts.
            </p>
          </div>
...
export default QuotesInsightful;
